...
def create_model(signal, noised_signal, min, max):
    ...
    np.save(open('s_preproc.npy','wb'),signals_shift)
    np.save(open('n_preproc.npy','wb'),noise_shift)
...
parser.add_argument('-up', '--upsampling', action='store', dest='upsampling', default = 1)
...
signals_shift, noise_shift, mapping, maxs, maxn = preprocessing(input_signals, input_noised_signals, min, max)
np.save(open('s_preproc.npy','wb'),signals_shift)
np.save(open('n_preproc.npy','wb'),noise_shift)
